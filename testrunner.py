...
def _test_wrapper(test_func):
    ...
    def wrapper(self):
        ...
        if interval <= 0:
            ...
            td = TimeoutDetector(interval, inspect.currentframe())
...
class TimeoutDetector(Bdb):
    __slots__ = ['_event', '_frame', '_timer']

    def __init__(self, interval, frame: FrameType):
        super().__init__()
        self._event = event = Event()
        self._frame = frame
        self._timer = Timer(interval, event.set)

    def do_clear(self, arg):
        pass

    def user_line(self, frame: FrameType):
        if frame != self._frame and self._event.is_set():
            raise TestTimeout
    ...
    def set_trace(self, frame: FrameType = None):
        self._timer.start()
        super().set_trace(self._frame)
...
class AttributeGuesser:
    ...
    def __guess_attribute(self, obj: Any, name: str):
        ...
        matches = difflib.get_close_matches(name, attributes, n=1, cutoff=0.7)
        if not matches:
            if self.__fail:
...
class TestMaster:
    ...
    def __init__(self,
        ...
        parser.add_argument("--disable-tk",
                            help="Whether or not to display output in JSON format.",
                            action='store_true',
                            default=disable_tk)
